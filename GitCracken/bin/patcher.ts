...
export class Patcher {
  ...
  private patchDirWithPro(): void {
    ...
    const searchValue =
      ...
      //'return JSON.parse((0,X.default)(Buffer.from(K,"base64").toString("utf8"),Buffer.from(q.secure,"base64")).toString("utf8"))}';
      /return (JSON\.parse\(.*?\.secure,"base64"\)\)\.toString\("utf8"\)\))\};/;
    const replaceValue =
